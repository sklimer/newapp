...
class TelegramWebAppMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Check for flexible paths that should allow external access
        is_flexible_path = any(path.startswith(flexible_path) for flexible_path in self.flexible_cart_paths)

        # If it's a public path or flexible path, allow access without Telegram restrictions
        if is_public_path or is_flexible_path:
