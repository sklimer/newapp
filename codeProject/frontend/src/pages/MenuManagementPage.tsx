...
import { menuApi, userApi } from '../api/api';
...
const MenuManagementPage: React.FC = () => {
  const [mainTab, setMainTab] = useState<'menu' | 'orders' | 'clients'>('menu');
  ...
  const loadCustomers = async () => {
    try {
      ...
      const data = response.data;
      // Ensure we're working with an array
      const customersData = Array.isArray(data) ? data : (data?.users || []);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 font-semibold ${mainTab === 'menu' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
          onClick={() => setMainTab('menu')}
        >
          Меню
        </button>
        <button
          className={`py-2 px-4 font-semibold ${mainTab === 'orders' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
          onClick={() => setMainTab('orders')}
        >
          Заказы
        </button>
        <button
          className={`py-2 px-4 font-semibold ${mainTab === 'clients' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
      ...
      {/* Orders Tab - Temporary text */}
      {mainTab === 'orders' && (
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-xl font-semibold mb-4">Тут будут заказы</h2>
        </div>
      )}

      {/* Clients Tab - Customer list */}
      {mainTab === 'clients' && (
        <div className="bg-white p-6 rounded-lg shadow overflow-x-auto">
          <h2 className="text-xl font-semibold mb-4 pb-2 border-b">Список клиентов</h2>
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID (telegram_id)</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Пригласил</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Кол-во заказов</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Сумма заказов</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Бонусы</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {customers.length > 0 ? (
                customers.map((customer) => (
                  <tr key={customer.id}>
                    <td className="px-6 py-4 whitespace-nowrap">{customer.telegram_id}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{customer.referred_by || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{customer.order_count}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{customer.total_spent.toFixed(2)}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{customer.bonus_balance.toFixed(2)}</td>
                ...
                <tr>
                  <td colSpan={10} className="px-6 py-4 text-center text-gray-500">
                    {customers.length === 0 ? 'Нет данных о клиентах' : 'Загрузка...'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* Menu Tab - Sub-tabs for Categories and Products */}
      {mainTab === 'menu' && (
        <div>
          {/* Sub-tabs */}
          <div className="flex border-b mb-6">
            <button
              className={`py-2 px-4 font-semibold ${subTab === 'categories' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
              onClick={() => setSubTab('categories')}
            >
              Категории
            </button>
            <button
              className={`py-2 px-4 font-semibold ${subTab === 'products' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
